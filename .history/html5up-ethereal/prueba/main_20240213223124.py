from pyscript import display, worker
import random
...
def random_response(language):
    if language == 1:
        return res_esp[random.randint(0, len(res_esp)-1)]["bot_response"]
        
    elif language == 2:
        return res_eng[random.randint(0, len(res_eng)-1)]["bot_response"]
...
def process_input(input_msg, language):
    if input_msg != "":
        ...
        return choose_response(set(tokenized_words), language)
    else:
...
while True:
    if language == 1:
        input_msg = process_input()
        display(process_input(input_msg, language), target="mpl")
        print("\n")
